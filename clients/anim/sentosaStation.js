...
var nearFieldGridRadius = Math.ceil(nearFieldRadius/gridSize);//30/3=10
...
function init() {
	...
	clients = [];
	var v = 0.2;
	data.matrix_sentosa.forEach(function (times, index) {
...
var activeCount, activeSelectedCount,countone,counttwo,countthree,countfour,countfive,countsix,countseven,counteight;
var temp=0;
var P1=[0,0,0,0,0,0,0];
var P2=[0,0,0,0,0,0,0];
var P3=[0,0,0,0,0,0,0];
var P4=[0,0,0,0,0,0,0];
var P5=[0,0,0,0,0,0,0];
var P6=[0,0,0,0,0,0,0];
var P7=[0,0,0,0,0,0,0];
var P8=[0,0,0,0,0,0,0];
var label=[0,0,0,0,0,0,0];
...
function updateData() {
	...
	data.matrix_sentosa.forEach(function (times, index) {
		...
		if (client.point !== undefined) {
			...
			if (client.point===1) countone++;
			if (client.point===2) counttwo++;
			if (client.point===3) countthree++;
			if (client.point===4) countfour++;
			if (client.point===5) countfive++;
			if (client.point===6) countsix++;
			if (client.point===7) countseven++;
			if (client.point===8) counteight++;
		}
		

		//console.log(clients[index],currentTime,point,data.points[point].x,data.points[point].room);
	});
...
function renderInfos() {
	...
	m = (m+100+'').substr(1);
	$('#timer').html('Time:  '+h+':'+m);
	var text = 'Active MAC-Address ID: '+activeCount;
	var Pointcount=[countone,"-",counttwo,"-",countthree,"-",countfour,"-",countfive,"-",countsix,"-",countseven,"-",counteight];
	if (selectedCount > 0) text += '<br><span style="color:#ee5000">Selected MAC-Address ID: '+activeSelectedCount+'</span>';
	$('#statistics').html(text);
	
	document.getElementById("beach").innerHTML='Beach Station:'+countone;
	document.getElementById("Imbiah").innerHTML='Imbiah Station:'+counttwo;
	document.getElementById("Sentosa").innerHTML='Sentosa Station:'+countfive;
	document.getElementById("Wing").innerHTML='Wing of Time:'+counteight;
	document.getElementById("Water").innerHTML='Waterfront Station:'+countseven;
	if (temp!==m) {P1.push(countone); 
					P1.shift();
					P2.push(counttwo);
					P2.shift();
					P5.push(countfive);
	...
	if (temp!==m) {label.push(h+':'+m);
					label.shift()};
	//console.log(temp,m,label,temp!==m);
	temp=m;
	...
	$('#sliderInner').css('left', (currentTime-1440));
...
var data={
	...
	datasets:[{
	  label:"Point1 Count",
	  fillColor:"rgba(55,131,237,0.7)",
	  strokeColor: "rgba(220,220,220,1)",
      pointColor: "rgba(7,7,7,0.7)",
...
var options = {
    ...
    title:{display:true,text:'Beach Station'},
	...
	clients.forEach(function (client) {
		...
		if (client.x === undefined) {
			// Punkt ist neu, also finde einen neuen Ort zum settlen
				//Dot is new, so find a new place to settle
			...
			if (!client.settled) {
				// Kommt gerade von wo anders her
					//Comes straight from where else
				...
				if (r < nearFieldRadius) {
					...
					//Is the new place still available?
					if (grid[xn][yn]) {
						grid[xn][yn] = false;
...
function renderCanvas() {
	...
	context.strokeStyle = 'rgba(255,255,0,'+a+')';
